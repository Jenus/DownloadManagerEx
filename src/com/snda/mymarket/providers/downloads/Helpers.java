...
import com.snda.mymarket.providers.downloads.Downloads.RequestHeaders;
...
public class Helpers {
	 ...
	 */
	public static boolean startViewIntent(Context context, long id,
			int intentFlags) {
		final Intent intent = buildViewIntent(context, id);
		if (intent == null) {
		...
		try {
			...
			Log.w(Constants.TAG, "Failed to start " + intent + ": " + e);
			return false;
		}
	}
	...
	private static Intent buildViewIntent(Context context, long id) {
		...
		try {
			...
			if ("application/vnd.android.package-archive".equals(mimeType)) {
				...
				final Uri remoteUri = getCursorUri(cursor, Downloads.COLUMN_URI);
				intent.putExtra(Intent.EXTRA_ORIGINATING_URI, remoteUri);
				intent.putExtra(Intent.EXTRA_REFERRER,
						getRefererUri(context, id));
				...
				intent.setDataAndType(ContentUris.withAppendedId(
						Downloads.ALL_DOWNLOADS_CONTENT_URI, id), mimeType);
			} else {
				intent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
				intent.setDataAndType(localUri, mimeType);
			}
	...
	private static int getOriginatingUid(Context context, long id) {
		final Uri uri = ContentUris.withAppendedId(Downloads.ALL_DOWNLOADS_CONTENT_URI,
				id);
	...
	public static String generateSaveFile(Context context, String url,
			String hint, long currentBytes, String contentDisposition,
			String contentLocation, String mimeType, int destination,
			long contentLength, boolean isPublicApi)
			throws GenerateSaveFileError {
		checkCanHandleDownload(context, mimeType, destination, isPublicApi);
		if (destination == Downloads.DESTINATION_FILE_URI) {
			return getPathForFileUri(url, hint, currentBytes,
					contentDisposition, contentLocation, mimeType, destination,
					contentLength);
		} else {
	...
	private static String getPathForFileUri(String url, String hint,
			long currentBytes, String contentDisposition,
			String contentLocation, String mimeType, int destination,
		...
		if (path.endsWith("/")) {
			...
			if (file.exists() && file.length() != currentBytes) {
