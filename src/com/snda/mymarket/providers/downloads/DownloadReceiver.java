...
public class DownloadReceiver extends BroadcastReceiver {
	...
	private void hideNotification(Context context, long id) {
		...
		if (Downloads.isStatusCompleted(status)
				&& (visibility == Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED || visibility == Request.VISIBILITY_VISIBLE_NOTIFY_ONLY_COMPLETION)) {
	 ...
	 */
	public static void openDownload(Context context, long id, final boolean bToast ) {
		if (!Helpers.startViewIntent(context, id,
		...
		try {
			if (cursor.moveToFirst()) {
				...
				isPublicApi = getInt(cursor,
						Downloads.COLUMN_IS_PUBLIC_API) != 0;
			} else {
